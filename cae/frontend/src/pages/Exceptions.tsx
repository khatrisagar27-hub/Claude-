...
export default function Exceptions() {
  ...
  }, [companyId, page, severity, status]);
    ...
    <div className="space-y-4">
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full">
          <thead>
            <tr>
              {['Severity', 'Title', 'Business Area', 'Risk Category', 'Financial Impact', 'Status', 'Detected'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center py-12 text-gray-500">No exceptions found</td></tr>
            ) : exceptions.map(exc => (
              <tr key={exc.id} className="table-row">
                <td className="table-cell"><SeverityBadge severity={exc.severity} /></td>
                <td className="table-cell max-w-xs">
                  <Link to={`/exceptions/${exc.id}`} className="text-blue-400 hover:text-blue-300 hover:underline line-clamp-2">
                    {exc.title}
                  </Link>
                </td>
                <td className="table-cell text-gray-400">{exc.business_area}</td>
                <td className="table-cell text-gray-400">{exc.risk_category}</td>
                ...
                <td className="table-cell text-gray-500 text-xs">
                  {new Date(exc.detected_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' })}
                </td>
              </tr>
            ))}
          </tbody>
